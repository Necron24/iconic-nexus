...
export default async function AccountSettingsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ count: ownedProjects }, { count: activeTests }, { count: openDisputes }] = await Promise.all([
    supabase.from("projects").select("id", { count: "exact", head: true }).eq("owner_id", user.id),
    supabase.from("campaign_members").select("id", { count: "exact", head: true }).eq("tester_id", user.id).in("status", ["joined", "in_progress", "submitted"]),
    supabase.from("feedback_disputes").select("id", { count: "exact", head: true }).eq("opened_by", user.id).eq("status", "open")
  ]);

  return <div className="max-w-3xl">
    <p className="text-sm font-bold uppercase tracking-[.2em] text-cyan">Account security</p>
    <h2 className="mt-2 text-3xl font-black">Account and data</h2>
    <p className="mt-2 text-soft">Manage permanent account actions carefully.</p>

    <section className="card mt-6 p-6">
      <div className="flex items-start gap-3"><Download className="mt-1 text-cyan"/><div><h3 className="text-xl font-black">Your account summary</h3><p className="mt-2 text-sm text-soft">Signed in as {user.email}</p></div></div>
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl bg-white/5 p-4"><p className="text-xs text-soft">Owned projects</p><p className="mt-1 text-2xl font-black">{ownedProjects || 0}</p></div>
        <div className="rounded-xl bg-white/5 p-4"><p className="text-xs text-soft">Active tests</p><p className="mt-1 text-2xl font-black">{activeTests || 0}</p></div>
        <div className="rounded-xl bg-white/5 p-4"><p className="text-xs text-soft">Open disputes</p><p className="mt-1 text-2xl font-black">{openDisputes || 0}</p></div>
      </div>
    </section>

    <section className="mt-6 rounded-2xl border border-red-400/30 bg-red-400/[0.06] p-6">

...
export default async function TestingWorkspace({ params, searchParams }: { params: Promise<{ membershipId: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <div>
    ...
    <div className="card mt-6 p-6">
      ...
      {(sessions ?? []).length === 0 ? (
        <p className="mt-3 text-soft">No sessions logged yet.</p>
      ) : (
        <div className="mt-4 divide-y divide-white/10">
          {(sessions ?? []).map((session) => (
            <Link
              key={session.id}
              href={`/dashboard/testing/${membershipId}/sessions/${session.id}`}
              className="group flex gap-4 py-4 transition hover:bg-white/[0.03] sm:items-center sm:justify-between sm:px-3"
            >
              <div className="min-w-0 flex-1">
                <p className="font-bold group-hover:text-cyan">
                  {session.device_name}
                  {session.os_version ? ` · ${session.os_version}` : ""}
                </p>
                {session.notes && (
                  <p className="mt-2 line-clamp-2 text-sm text-soft">{session.notes}</p>
                )}
                <p className="mt-2 text-xs font-bold text-cyan">View session details →</p>
              </div>
              <span className="flex shrink-0 items-center gap-2 text-sm text-cyan">
                <Clock3 size={15} />
                {session.minutes_tested} min
              </span>
            </Link>
          ))}
        </div>

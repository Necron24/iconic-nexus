...
export default async function ForgotPasswordPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <section className="container-page grid min-h-[70vh] place-items-center py-14"><div className="card w-full max-w-md p-7">
    ...
    <form action={requestPasswordReset} className="mt-7 space-y-5"><label><span className="label">Email address</span><input name="email" type="email" autoComplete="email" required className="field" placeholder="you@example.com" /></label><TurnstileWidget action="password_reset" /><button className="btn-primary w-full">Send reset link</button></form>

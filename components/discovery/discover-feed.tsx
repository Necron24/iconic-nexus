...
export function DiscoverFeed({
  ...
  personalization?: { role: string; platforms: string[]; hasSignals: boolean } | null;
}) {
  ...
  const toggleFollow = useCallback(async (project: DiscoverProject) => {
    if (!currentUserId) {
      window.location.href = `/login?error=${encodeURIComponent("Please log in to follow projects.")}&next=${encodeURIComponent("/discover")}`;
      return;
    }
    if (project.owner_id === currentUserId) return;
    const following = followedIds.has(project.id);
    setFollowedIds((current) => {
      const next = new Set(current);
      if (following) next.delete(project.id); else next.add(project.id);
      return next;
    });
    const result = following
      ? await supabase.from("project_follows").delete().eq("project_id", project.id).eq("profile_id", currentUserId)
      : await supabase.from("project_follows").insert({ project_id: project.id, profile_id: currentUserId });
    if (result.error) {
      setFollowedIds((current) => {
        const next = new Set(current);
        if (following) next.add(project.id); else next.delete(project.id);
        return next;
      });

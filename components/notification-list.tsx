...
export function NotificationList({ notifications }: { notifications: NotificationItem[] }) {
  ...
  return <div className="space-y-3">{items.map(n => <button type="button" disabled={isPending && !n.is_read} onClick={() => open(n)} key={n.id} className={`card block w-full p-5 text-left transition hover:bg-white/[0.06] ${n.is_read ? "opacity-70" : "border-lime/30 bg-lime/[0.04]"}`}><div className="flex items-start justify-between gap-4"><div><p className="font-black">{n.title}</p><p className="mt-2 text-sm leading-6 text-soft">{n.message}</p></div>{!n.is_read && <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-lime"/>}</div><p className="mt-3 text-xs text-soft">{new Date(n.created_at).toLocaleString("en-ZA", { dateStyle: "medium", timeStyle: "short", timeZone: "Africa/Johannesburg" })}</p></button>)}</div>;
}

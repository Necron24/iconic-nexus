...
export function NotificationPopover({ unread, notifications }: { unread: number; notifications: HeaderNotification[] }) {
  ...
  return <div ref={panelRef} className="relative">
    ...
    {open && <div role="dialog" aria-label="Notifications" className="fixed inset-x-3 top-[calc(env(safe-area-inset-top)+92px)] z-[80] flex max-h-[calc(100dvh-108px)] flex-col overflow-hidden rounded-[24px] border border-white/15 bg-[#101827]/95 shadow-[0_28px_90px_rgba(0,0,0,.48)] backdrop-blur-2xl sm:absolute sm:inset-x-auto sm:right-0 sm:top-[calc(100%+12px)] sm:max-h-none sm:w-[390px]">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/10 px-4 py-4"><div className="min-w-0"><p className="font-black text-white">Notifications</p><p className="mt-0.5 truncate text-xs text-soft">{displayedUnread > 0 ? `${displayedUnread} unread` : "You are all caught up"}</p></div>{displayedUnread > 0 && <button type="button" disabled={isPending} onClick={markAll} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-soft transition hover:bg-white/10 hover:text-white disabled:opacity-50"><CheckCheck size={15}/>Mark read</button>}</div>
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-2 sm:max-h-[min(62vh,470px)]">{items.length === 0 ? <div className="px-5 py-10 text-center"><Bell className="mx-auto text-soft" size={24}/><p className="mt-3 font-bold">Nothing new yet</p><p className="mt-1 text-sm text-soft">Important activity will appear here.</p></div> : <div className="space-y-1">{items.map(n => <button key={n.id} type="button" onClick={() => openNotification(n)} className={`block w-full rounded-2xl px-4 py-3 text-left transition hover:bg-white/[0.07] ${n.is_read ? "opacity-70" : "bg-lime/[0.045]"}`}><div className="flex items-start gap-3"><span className={`mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${n.is_read ? "bg-white/20" : "bg-lime"}`}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{n.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-soft">{n.message}</p><p className="mt-2 text-[11px] text-soft/80">{new Date(n.created_at).toLocaleString("en-ZA", { dateStyle: "medium", timeStyle: "short", timeZone: "Africa/Johannesburg" })}</p></div></div></button>)}</div>}</div>
      <div className="shrink-0 border-t border-white/10 p-3"><Link href="/dashboard/notifications" onClick={() => setOpen(false)} className="block rounded-xl bg-white/5 px-4 py-3 text-center text-sm font-black text-white transition hover:bg-white/10">View all notifications</Link></div>
    </div>}
  </div>;
}

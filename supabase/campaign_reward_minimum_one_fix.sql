-- Iconic Nexus: enforce a simple minimum campaign reward of 1 credit.
-- Run this after all earlier campaign/escrow SQL files.
...
begin
  ...
  if p_reward_credits < 1 or p_reward_credits > 1000 then raise exception 'Reward amount must be between 1 and 1000 credits.'; end if;
...
begin
  ...
  if p_tester_goal < v_joined_count then
    raise exception 'Tester goal cannot be lower than the number of joined testers (%).', v_joined_count;
  end if;

  v_desired_total := p_tester_goal * p_reward_credits;
  if v_desired_total < v_campaign.spent_credits then
    raise exception 'The new campaign budget cannot be lower than credits already paid.';
  end if;
